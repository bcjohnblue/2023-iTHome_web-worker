...
export const gaussianBlur = (percent = 0) => {
  ...
  // cv.cvtColor(binaryMat, binaryMat, cv.COLOR_GRAY2RGBA);

  // Bitwise AND with the original image to remove bright spots
  cv.bitwise_and(inputMat, inputMat, outputMat, binaryMat);
  // cv.bitwise_and(inputMat, inputMat, outputMat, binaryMat);

  cv.cvtColor(outputMat, outputMat, cv.COLOR_RGBA2GRAY);

  cv.threshold(outputMat, outputMat, 250, 255, cv.THRESH_TOZERO_INV);
